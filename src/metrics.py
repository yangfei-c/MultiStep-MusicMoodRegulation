...
def check_arrays(predictions, targets, output_dim: int, name: str) -> tuple[np.ndarray, np.ndarray]:
    ...
    if predictions.shape != targets.shape or predictions.ndim != 2 or predictions.shape[1] != output_dim:
        raise ValueError(f"{name} predictions 和 targets 应为 [N,{output_dim}]，实际为 {predictions.shape}、{targets.shape}")
    if predictions.shape[0] < 2: raise ValueError(f"{name} 至少需要 2 个样本")
    if not np.isfinite(predictions).all() or not np.isfinite(targets).all(): raise ValueError(f"{name} 输入包含 NaN 或 Inf")
...
def compute_mtg_metrics(logits, targets) -> dict:
    ...
    if not np.isin(targets, (0.0, 1.0)).all(): raise ValueError("MTG targets 必须是二值标签")
    ...
    for index in range(183):
        ...
        if label_targets.sum() > 0: per_label_ap[index] = average_precision_score(label_targets, label_scores)
        if np.unique(label_targets).size == 2: per_label_auc[index] = roc_auc_score(label_targets, label_scores)
    ...
    metrics["tag_score"] = float(np.mean(group_maps)) if np.isfinite(group_maps).all() else float("nan")
    metrics["tag_roc_auc"] = float(np.mean(group_aucs)) if np.isfinite(group_aucs).all() else float("nan")
    metrics["overall_map"] = float(np.nanmean(per_label_ap)) if np.isfinite(per_label_ap).any() else float("nan")
    metrics["overall_roc_auc"] = float(np.nanmean(per_label_auc)) if np.isfinite(per_label_auc).any() else float("nan")
    metrics["valid_ap_labels"] = int(np.isfinite(per_label_ap).sum())
    metrics["valid_auc_labels"] = int(np.isfinite(per_label_auc).sum())
    return metrics
...
def concordance_correlation_coefficient(targets: np.ndarray, predictions: np.ndarray, eps: float = 1.0e-12) -> float:
    ...
    if denominator <= eps: return 1.0 if np.allclose(targets, predictions, atol=eps, rtol=0.0) else 0.0
...
def compute_va_metrics(predictions, targets) -> dict[str, float]:
    ...
    return {
        "valence_ccc": ccc[0],
        "arousal_ccc": ccc[1],
        "va_score": float(np.mean(ccc)),
        "valence_rmse": float(rmse[0]),
        "arousal_rmse": float(rmse[1]),
        "mean_rmse": float(rmse.mean()),
        "valence_mae": float(mae[0]),
        "arousal_mae": float(mae[1]),
